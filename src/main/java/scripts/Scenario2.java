...
public class Scenario2 extends Keywords {
	...
	public void Semiautorun2(WebDriver driver) throws AWTException {
		...
		action1.build().perform();
		 ...
		 wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//h4[contains(@class, 'ves_topbar_align') and contains(text(), '"+MasterPlanFile+"')]")));
			
			String Planname = driver.findElement(By.xpath("//*[@class='ves_topbar_align']")).getText();
			
			if (Planname.contains(MasterPlanFile)) {
		...
		String fileName = System.getProperty("user.dir")+"\\uploads\\02Master_" + path + ".xlsx";

		String autoITExecutable = System.getProperty("user.dir")+"\\driver\\MasterDownloadfile.exe " + fileName;
...
//		}
        ...
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//h4[contains(@class, 'ves_topbar_align') and contains(text(), '"+TestPlanFile+"')]")));
		
		String Planname1 = driver.findElement(By.xpath("//*[@class='ves_topbar_align']")).getText();
		
		if (Planname1.contains(TestPlanFile)) {
			System.out.println("Correct plan is opened");
		} else {
			System.out.println("Different plan is opned");
		}

		List<WebElement> tot=driver.findElements(By.xpath("//div[text()='ABP']//following::canvas"));
...
//		// 24 completed
		...
		String TestfileName = System.getProperty("user.dir")+"\\uploads\\02Test_" + Testpath + ".xlsx";
